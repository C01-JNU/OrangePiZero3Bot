...
- **Census 变换**：将灰度图转换为 Census 特征图，可选 CPU 或 GPU（Vulkan）实现，通过配置文件切换
- **硬件监控**：实时监控 CPU 温度、使用率、内存等信息，通过独立 UDP 流发送
- **网络传输**：轻量级 UDP 分片传输，支持多流（左校正图、左右 Census 图、原始拼接图、硬件状态）
...
#### 界面功能
- **2x2 网格显示**：左校正图、左 Census 图、右 Census 图、原始拼接图
